...
public class pane extends JPanel{
    ...
    public void paintComponent(Graphics g){
        ...
        while(i<time){
            ...
            //Line2D line1 = new Line2D.Double(spot[i][j]*5+10,spot[i][j+1]*5+10,spot[i][j]*5+10,spot[i][j+1]*5+10);
            //Line2D line = new Line2D.Double(spot[i][j]*10,spot[i][j+1]*10,spot[i][j]*10,spot[i][j+1]*10);
            g2.draw(ellipse);
            i++;

        }
        float x1 = (weight[0]/weight[1])*5+100;
        float x2 = (weight[0]/weight[2])*5+100;//此線斜率:-x2/x1
        float x11 = x1+1000;
